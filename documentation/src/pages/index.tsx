import Link from '@docusaurus/Link';
import useDocusaurusContext from '@docusaurus/useDocusaurusContext';
import { Circles, Features, QuickStart } from '@site/src/components';
import Layout from '@theme/Layout';

export default function Home() {
  const { siteConfig } = useDocusaurusContext();

  return (
    <Layout
      description="A React Native template for building solid applications, using JavaScript or Typescript (YOU choose)."
      title={`Light Simple and Scalable ${siteConfig.title} by TheCodingMachine`}
      wrapperClassName="relative! overflow-hidden!"
    >
      <div className="relative! max-w-7xl! m-auto! w-full! opacity-40!">
        <div
          className="animate-blob! opacity-80! absolute! top-0! -right-10! md:top-0! md:right-0! bg-linear-to-r! from-indigo-600! via-blue-700! to-blue-400! h-52.5! w-52.5! md:h-87.5! md:w-87.5! lg:h-95.5! lg:w-95.5!"
          style={{ borderRadius: '30% 70% 67% 33% / 64% 30% 70% 36%' }}
        />
        <div
          className="animate-blob-delay-1! opacity-80! absolute! top-0! right-32! md:top-62.5! md:right-1/4! lg:right-60! bg-linear-to-r! from-red-600! via-pink-700! to-pink-400! h-45! w-45! md:h-57.5! md:w-57.5! lg:h-70.5! lg:w-70.5!"
          style={{ borderRadius: '30% 70% 67% 33% / 64% 30% 70% 36%' }}
        />
      </div>

      <div className="backdrop-blur-xl!">
        <div className="relative! max-w-7xl! m-auto!">
          <div className="absolute! top-200!">
            <Circles animate="animate-circle-delay-1" />
          </div>
          <div className="absolute! top-100! right-0!">
            <Circles color="white" />
          </div>
          <HomepageHeader />
        </div>

        <main className="relative!">
          <div className="relative! max-w-7xl! m-auto!">
            <Features />
          </div>

          <svg
            className="-mb-px! bottom-0! w-full! sm:-mt-20! fill-red-400!"
            viewBox="0 0 1440 320"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M0,320L60,272C120,224,240,128,360,128C480,128,600,224,720,234.7C840,245,960,171,1080,122.7C1200,75,1320,53,1380,42.7L1440,32L1440,320L1380,320C1320,320,1200,320,1080,320C960,320,840,320,720,320C600,320,480,320,360,320C240,320,120,320,60,320L0,320Z"
              fill="current"
              fillOpacity="1"
            />
          </svg>
          <QuickStart />
          <svg
            className="-mb-px! pointer-events-none! absolute! bottom-0! sm:bottom-42.25! lg:bottom-67.25! w-full! sm:-mt-20! fill-neutral-100! dark:fill-neutral-900!"
            viewBox="0 0 1440 320"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M0,288L48,272C96,256,192,224,288,197.3C384,171,480,149,576,165.3C672,181,768,235,864,250.7C960,267,1056,245,1152,250.7C1248,256,1344,288,1392,304L1440,320L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"
              fill="current"
              fillOpacity="1"
            />
          </svg>
        </main>
      </div>
    </Layout>
  );
}

function HomepageHeader() {
  const { siteConfig } = useDocusaurusContext();

  return (
    <header>
      <div className="bottom-0! w-screen! left-1/2! top-24! ml-3! mt-10! pt-0! md:pt-0! sm:-ml-5! md:ml-auto! flex! justify-end! right-12! md:top-auto! md:left-auto! md:-right-12! md:justify-center! absolute! md:w-2/3! sm:top-20! m-auto! lg:w-1/2!">
        <img
          alt="tom"
          className="dark:block! pointer-events-none! hidden! object-contain! z-10! w-full! md:w-3/4!"
          src="./img/tom_dark.webp"
        />

        <img
          alt="tom"
          className="dark:hidden! pointer-events-none! sm:block! object-contain! z-10! w-full! md:w-3/4!"
          src="./img/tom_light.webp"
        />
      </div>

      <div className="px-5! sm:px-12! md:px-16! relative mx-auto pt-10! md:pt-24!">
        <h1 className="text-[32px]! sm:text-[40px]! mt-10! lg:mt-16! md:mt-0! md:text-[45px]! lg:text-[63px]! leading-10! md:leading-8! font-black! text-indigo-700!">
          {'The React Native'}
          <span className="font-extralight! mt-4! md:mt-8! lg:mt-0! text-[66px]! sm:text-[83px]! md:text-[92px]! lg:text-9xl! block! text-red-500! dark:text-red-700!">
            {'Boilerplate'}
          </span>
        </h1>
        <p className="mt-6! pr-20! sm:pr-0! sm:mt-12! lg:mt-5! w-full! sm:w-2/3! md:w-1/2! text-sm! font-bold! ml-1! text-slate-700! dark:text-white! mb-0!">
          {siteConfig.tagline}
          <span className="ml-1! text-slate-400!">
            {
              'Explore the optimal React Native boilerplate for your project, featuring a straightforward architecture founded on the principle of Separation of Concerns. Join our vibrant community and watch it flourish.'
            }
          </span>
        </p>

        <div className="py-10">
          <Link
            className="transition-all! ease-in! hover:bg-red-700! hover:text-white! text-md! text-white! font-bold! py-3! px-4! rounded! bg-red-500! shadow-lg! shadow-red-500/50!"
            to="/docs/getting-started"
          >
            {'Get started'}
            <span className="ml-3">{'🚀'}</span>
          </Link>
        </div>
      </div>
    </header>
  );
}
