...
|-----------------------|------------------------------|
| **`font_`**sizesValue | { fontSize: **sizeValue** }  |
| **`text_`**colorsKey  | { color: **colorsValue** }   |

## Static fonts
Some fonts styles, such as the `fontWeight` property of a text style, don't require dynamic generation.
To accommodate this, we offer a `static/fonts.ts` file that houses static styles for fonts.
